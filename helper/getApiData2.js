...
const getApiData2 = async (matchId) => {
  ...
  try {
    ...
    const bookmaker =
      data.bookmakers &&
      Array.isArray(data.bookmakers) &&
      data.bookmakers.length > 0
        ? data.bookmakers.filter((x) => x.marketName === "Bookmaker").length > 0
          ? data.bookmakers.filter((x) => x.marketName === "Bookmaker")[0]
              .runners
          : []
        : [];
    const bookmakerStatus =
      data.bookmakers && data.bookmakers.length > 0
        ? data.bookmakers.find((x) => x.marketName === "Bookmaker")?.status ||
          "SUSPEND"
        : "SUSPEND";
